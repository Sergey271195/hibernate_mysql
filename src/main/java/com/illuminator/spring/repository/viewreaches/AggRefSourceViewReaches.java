...
@Repository
public interface AggRefSourceViewReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM ref_source_views WHERE counterid = :id AND date BETWEEN :startDate " +
