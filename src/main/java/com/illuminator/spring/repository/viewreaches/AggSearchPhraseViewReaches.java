...
@Repository
public interface AggSearchPhraseViewReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM search_phrase_views WHERE counterid = :id AND date BETWEEN :startDate " +
