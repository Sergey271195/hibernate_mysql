...
@Repository
public interface AggSocNetworkViewReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM social_network_views WHERE counterid = :id AND date BETWEEN :startDate " +
