...
@Repository
public interface AggSearchPhrasePriceReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM search_phrase_prices WHERE counterid = :id AND date BETWEEN :startDate " +
