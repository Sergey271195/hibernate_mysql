...
@Repository
public interface AggSearchEnginePriceReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM search_engine_prices WHERE counterid = :id AND date BETWEEN :startDate " +
