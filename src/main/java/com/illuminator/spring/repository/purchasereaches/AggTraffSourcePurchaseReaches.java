...
@Repository
public interface AggTraffSourcePurchaseReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM traff_source_purchases WHERE counterid = :id AND date BETWEEN :startDate " +
