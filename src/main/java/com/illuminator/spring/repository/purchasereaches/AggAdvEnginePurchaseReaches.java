...
@Repository
public interface AggAdvEnginePurchaseReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM adv_engine_purchases WHERE counterid = :id AND date BETWEEN :startDate " +
