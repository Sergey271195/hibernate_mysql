...
@Repository
public interface AggRefSourcePurchaseReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {

    @Query(value = "SELECT r.count as count, s.name as sourceName FROM " +
            "(SELECT sum(reaches) as count, sourceid " +
            "FROM ref_source_purchases WHERE counterid = :id AND date BETWEEN :startDate " +
            "AND :endDate GROUP BY sourceid) " +
            "r LEFT JOIN ref_source s ON r.sourceid = s.id ORDER BY count DESC", nativeQuery = true)
