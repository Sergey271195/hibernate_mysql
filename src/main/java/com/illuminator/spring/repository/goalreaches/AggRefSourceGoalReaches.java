...
@Repository
public interface AggRefSourceGoalReaches extends JpaRepository<com.illuminator.db.entity.goal.AdvEngineGoalReaches, Long> {
    ...
    @Query(value = "SELECT r.count as count, s.name as sourceName, a.name as goalName FROM " +
            ...
            "FROM ref_source_goal WHERE counterid = :id AND date BETWEEN :startDate " +
            "AND :endDate GROUP BY goalid, sourceid) " +
            "r LEFT JOIN goal a ON r.goalId = a.id " +
            "LEFT JOIN ref_source s ON r.sourceid = s.id ORDER BY goalName, count DESC", nativeQuery = true)
